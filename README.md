...
## العمل بلا إنترنت (الإقلاع لا يطلب الشبكة)

- **كل البيانات محلية:** Dexie/IndexedDB — القراءة والكتابة والحسابات والتذكيرات تعمل بلا شبكة إطلاقًا، ولا يوجد أي `fetch` في مسار الإقلاع.
- **بوابة التثبيت لا تسدّ الطريق:** إن كانت `navigator.onLine === false` يُضبط `data-gate="offline"` قبل أول رسم فيُفتح التطبيق مباشرة (التثبيت نفسه يحتاج شبكة، فلا نطالب بها إلا عند الضرورة). الاختيار لا يُحفظ: عند عودة الشبكة في زيارة جديدة تُمنح البوابة فرصتها.
- **حارس زمن (fail-open):** لو لم يصل `gatekeeper/app.js` (كاش قديم، 404، شبكة ضعيفة) يكشف `index.html` التطبيق تلقائيًا بعد 2.5 ثانية — فلا شاشة «جارٍ…» معلّقة أبدًا. السكربت يعلن `window.__gkReady` ويُلغي الحارس إذا وصل.
- **انقطع الاتصال أثناء البوابة؟** مستمِع `offline` يكشف التطبيق فورًا.
- **أصول البوابة داخل الـ precache** (`gatekeeper/app.js` + `style.css` + `index.html`) مع `NavigationRoute` ⇒ فتح من الأيقونة يعمل بلا إنترنت بعد أول زيارة.

## تعديل الالتزام (خانات معبّأة، لا نموذج فارغ)

- **المشكلة كانت:** بيانات Dexie تصل بعد أول رسم، وكان `ObligationForm` يُهيَّأ من `existing` مرة واحدة فقط — فيظهر نموذج التعديل فارغًا «كأنك تكتب التزامًا جديدًا».
- **الحل:** `useDataBundle.ready` صار صادقًا (استعلامات بلا قيمة ابتدائية)؛ وصفحة `/edit/:id` لا ترسم شيئًا قبل وصول البيانات (وتعرض «لم أجد هذا الالتزام» بدل نموذج فارغ إذا كان المعرّف محذوفًا)؛ وفي `ObligationForm` تُعاد تعبئة الخانات عند تغيّر مصدر النموذج أثناء الرسم (لا `useEffect` ولا وميض)، والمعيار هو **معرّف السجل** فقط حتى لا تضيع كتابات المستخدم عند أي تحديث حيّ.
- **التغطية:** `tests/obligation-edit.test.tsx` (6 اختبارات: كل الخانات، المبلغ/التاريخ/الوقت/ملاحظات/التكرار/الأولوية/التنبيهات، «إضافة» تبقى فارغة، معرّف غير موجود، الحفظ يعدّل نفس السجل بلا نسخة جديدة، وصول السجل بعد التركيب) + 8 اختبارات لسلوك البوابة بلا إنترنت في `tests/gatekeeper.test.ts`.
...
- **سياسة عدم الاتصال**: التطبيق يقلع ويعمل بالكامل بدون إنترنت — لا يوجد أي `fetch`/XHR/مورد خارجي في الكود (الخطوط والأيقونات مضمّنة ومحلية). الشبكة تُستخدم للضرورة فقط: (1) أول تحميل أو تحديث إصدار عبر Service Worker، (2) إنشاء اشتراك Web Push مرة واحدة (اختياري ويتطلب مفتاح VAPID). تظهر شارة «بدون إنترنت» في الشريط العلوي عند انقطاع الاتصال، وتفشل محاولات التحديث بصمت بدون أخطاء.
...
- **92 اختبارًا** في 12 ملفًا: محرك التكرار، الميزانية، الدفعات، النسخ الاحتياطي، البحث، الاقتراحات، بوابة التثبيت، نموذج التعديل، **دخان لكل الشاشات العشرين**، وحدود الخطأ.
- `ErrorBoundary` عام: أي خطأ غير متوقع يعرض واجهة طوارئ عربية (إعادة محاولة / عودة للرئيسية) بدل شاشة بيضاء — والبيانات محفوظة دائمًا على الجهاز.
